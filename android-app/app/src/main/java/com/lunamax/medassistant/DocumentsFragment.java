...
public final class DocumentsFragment extends BaseFragment {
    ...
    private void confirmSources(List<PendingSource> sources){if(sources==null||sources.isEmpty()){feedback("没有可识别的图片");return;}long generation=vision().currentGeneration();long bytes=0;for(PendingSource source:sources)bytes+=source.compressed.length();LinearLayout box=verticalForm();box.addView(infoLabel("将发送 "+sources.size()+" 页图片给 " + DeepSeekTransport.VISION_MODEL + "。原始文件仍在应用私有目录。"));box.addView(infoLabel("压缩后约 "+Math.max(1,bytes/1024)+" KB；不会自动发送健康档案。"));new MaterialAlertDialogBuilder(requireContext()).setTitle("确认 AI 图片识别").setView(box).setNegativeButton("保存，稍后识别",(d,w)->{if(vision().isCurrent(generation))saveWaiting(sources);}).setPositiveButton(assistant().hasKey()?"发送给视觉模型":"先保存待识别",(d,w)->{if(!vision().isCurrent(generation))return;if(assistant().hasKey()){queue.clear();queue.addAll(sources);queueIndex=0;recognizeNext();}else saveWaiting(sources);}).show();}
    ...
    private LunaDatabase.DocumentDraft draft(PendingSource source,String status){LunaDatabase.DocumentDraft d=new LunaDatabase.DocumentDraft();d.uri=source.original.getAbsolutePath();d.mimeType=source.sourceMime;d.sha256=safeHash(source.original);d.pageNo=source.page;d.compressedUri=source.compressed.getAbsolutePath();d.compressedSha256=safeHash(source.compressed);d.status=status;d.model=DeepSeekTransport.VISION_MODEL;d.promptVersion=VisionRepository.PROMPT_VERSION;return d;}
