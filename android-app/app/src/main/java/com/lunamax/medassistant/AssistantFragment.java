...
public final class AssistantFragment extends BaseFragment {
    ...
    private void testConnection(){if(!assistant().hasKey()){manageKey();return;}if(requestInFlight)return;requestInFlight=true;binding.assistantTestKey.setEnabled(false);binding.assistantStatus.setText("正在测试连接；不会发送个人资料或图片。");assistant().testConnection(new AssistantRepository.ConnectionCallback(){public void success(String message){requireActivity().runOnUiThread(()->{requestInFlight=false;binding.assistantTestKey.setEnabled(true);refresh();binding.assistantStatus.setText(message);});}public void failure(String message){requireActivity().runOnUiThread(()->{requestInFlight=false;binding.assistantTestKey.setEnabled(true);refresh();binding.assistantStatus.setText(message);});}});}
